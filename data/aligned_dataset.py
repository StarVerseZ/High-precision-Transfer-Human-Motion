...
class AlignedDataset(BaseDataset):
    def initialize(self, opt):
        ...
        if not opt.openpose_only:
            dir_src_pose = '_src_densepose'
            self.dir_src_pose = os.path.join(self.root, 'train' + dir_src_pose)
            self.src_pose_paths = sorted(make_dataset(self.dir_src_pose))

            dir_trg_pose = '_trg_densepose'
            self.dir_trg_pose = os.path.join(self.root, 'train' + dir_trg_pose)
            self.trg_pose_paths = sorted(make_dataset(self.dir_trg_pose))
    ...
    def __getitem__(self, index):
        ...
        ####
        src_pose_path = self.src_pose_paths[src_index]
        src_pose_path_b1 = self.src_pose_paths[src_index_b1]
        src_pose_path_b2 = self.src_pose_paths[src_index_b2]
        
        src_densepose,src_mask = self.get_image(src_pose_path, trg_img.size, params, input_type='densepose')
        src_densepose[2,:,:] = ((src_densepose[2,:,:] * 0.5 + 0.5) * 255 / 24 - 0.5) / 0.5
        src_openpose = self.get_image(self.src_openpose_paths[src_index], trg_img.size, params, input_type='openpose')
        src_pose = torch.cat((src_densepose, src_openpose))

        src_densepose_b1, src_mask_b1 = self.get_image(src_pose_path_b1, trg_img.size, params, input_type='densepose')
        src_densepose_b1[2,:,:] = ((src_densepose_b1[2,:,:] * 0.5 + 0.5) * 255 / 24 - 0.5) / 0.5
        src_openpose_b1 = self.get_image(self.src_openpose_paths[src_index_b1], trg_img.size, params, input_type='openpose')
        src_pose_b1 = torch.cat((src_densepose_b1, src_openpose_b1))

        src_densepose_b2, src_mask_b2 = self.get_image(src_pose_path_b2, trg_img.size, params, input_type='densepose')
        src_densepose_b2[2,:,:] = ((src_densepose_b2[2,:,:] * 0.5 + 0.5) * 255 / 24 - 0.5) / 0.5
        src_openpose_b2 = self.get_image(self.src_openpose_paths[src_index_b2], trg_img.size, params, input_type='openpose')
        src_pose_b2 = torch.cat((src_densepose_b2, src_openpose_b2))

        trg_pose_path = self.trg_pose_paths[trg_index]
        trg_densepose,trg_mask = self.get_image(trg_pose_path, trg_img.size, params, input_type='densepose')
        trg_densepose[2,:,:] = ((trg_densepose[2,:,:] * 0.5 + 0.5) * 255 / 24 - 0.5) / 0.5
        trg_openpose = self.get_image(self.trg_openpose_paths[trg_index], trg_img.size, params, input_type='openpose')
        trg_pose = torch.cat((trg_densepose, trg_openpose))

        trg_pose_path_b1 = self.trg_pose_paths[trg_index_b1]
        trg_densepose_b1, trg_mask_b1 = self.get_image(trg_pose_path_b1, trg_img.size, params, input_type='densepose')
        trg_densepose_b1[2,:,:] = ((trg_densepose_b1[2,:,:] * 0.5 + 0.5) * 255 / 24 - 0.5) / 0.5
        trg_openpose_b1 = self.get_image(self.trg_openpose_paths[trg_index_b1], trg_img.size, params, input_type='openpose')
        trg_pose_b1 = torch.cat((trg_densepose_b1, trg_openpose_b1))

        trg_pose_path_b2 = self.trg_pose_paths[trg_index_b2]
        trg_densepose_b2 ,trg_mask_b2 = self.get_image(trg_pose_path_b2, trg_img.size, params, input_type='densepose')
        trg_densepose_b2[2,:,:] = ((trg_densepose_b2[2,:,:] * 0.5 + 0.5) * 255 / 24 - 0.5) / 0.5
        trg_openpose_b2 = self.get_image(self.trg_openpose_paths[trg_index_b2], trg_img.size, params, input_type='openpose')
