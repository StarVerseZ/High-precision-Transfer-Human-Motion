...
class GAN(BaseModel):
    ...
    def initialize(self, opt):
        ...
        ##### define networks
        # Generator network
        # input_nc *2 for densepose and openpose, *3 for consecutive 3 frame
        self.netE_label = networks.define_E(opt.input_nc*2*3, opt.output_nc, opt.ngf,
        ...
        self.netA = networks.define_A(opt.output_nc*3, opt.output_nc, opt.n_downsample_global, gpu_ids=self.gpu_ids)
        # Discriminator network
        if self.isTrain:
            netD_input_nc = opt.input_nc*2*3 + opt.output_nc
    ...
    trg_mask):
        ...
        # Encode Inputs
        # 3 frames encode as l list
        src_label = self.encode_input(src_label)
        src_image = self.encode_input(src_image)
        trg_label = self.encode_input(trg_label)
        trg_image = self.encode_input(trg_image)
        src_mask = self.encode_input(src_mask)
        trg_mask = self.encode_input(trg_mask)

        src_template, trg_template = \
        self.encode_input([src_template, trg_template])

        # encode pose
        placeholder = Variable(torch.zeros_like(src_label[0])).cuda()
        src_label_ = torch.cat([src_label[0],src_label[1],src_label[2]], dim=1)
        src_label_b1 = torch.cat([src_label[1],src_label[2],placeholder], dim=1)
        src_label_b2 = torch.cat([src_label[2],placeholder,placeholder], dim=1)

        src_label_encoded = self.netE_label(src_label_)
        src_label_encoded_b1 = self.netE_label(src_label_b1)
        src_label_encoded_b2 = self.netE_label(src_label_b2)

        trg_label_ = torch.cat([trg_label[0],trg_label[1],trg_label[2]], dim=1)
        trg_label_b1 = torch.cat([trg_label[1],trg_label[2],placeholder], dim=1)
        trg_label_b2 = torch.cat([trg_label[2],placeholder,placeholder], dim=1)
        
        trg_label_encoded = self.netE_label(trg_label_)
        trg_label_encoded_b1 = self.netE_label(trg_label_b1)
        trg_label_encoded_b2 = self.netE_label(trg_label_b2)
        ...
        with torch.no_grad():
            ...
            src2trg_b1 = self.netDe(src_label_encoded_b1, trg_template_encoded, 'trg')
            src2trg_b2 = self.netDe(src_label_encoded_b2, trg_template_encoded, 'trg')

            trg2src = self.netDe(trg_label_encoded, src_template_encoded, 'src')
            trg2src_b1 = self.netDe(trg_label_encoded_b1, src_template_encoded, 'src')
            trg2src_b2 = self.netDe(trg_label_encoded_b2, src_template_encoded, 'src')

        # src_mask = src_mask.float()
        # trg_mask = trg_mask.float()

        src_image_ = torch.cat([src_image[0]*src_mask[0].float(), src_image[1]*src_mask[1].float(), src_image[2]*src_mask[2].float()], dim=1)
        src2trg_ = torch.cat([src2trg, src2trg_b1, src2trg_b2], dim=1)
        src2trg_mask = self.netA(src_image_, src2trg_)

        trg_image_ = torch.cat([trg_image[0], trg_image[1], trg_image[2]], dim=1)
        trg2src_ = torch.cat([trg2src*trg_mask[0].float(), trg2src_b1*trg_mask[1].float(), trg2src_b2*trg_mask[2].float()], dim=1)
        trg2src_mask = self.netA(trg2src_, trg_image_)
     ...
     # Fake Detection and Loss
        ...
        if not self.opt.no_vgg_loss:
            loss_G_VGG_src = self.criterionVGG(src_fake, src_image[0]) * self.opt.lambda_feat

        loss_G_VGG_trg = 0
        if not self.opt.no_vgg_loss:
            loss_G_VGG_trg = self.criterionVGG(trg_fake, trg_image[0]) * self.opt.lambda_feat
        ...
        loss_A_VGG = 0
        if not self.opt.no_vgg_loss:
            loss_A_VGG = self.criterionVGG(trg2src_mask, trg_image[0]) * self.opt.lambda_feat
    ...
    def inference(self, src_label, src_image, trg_template, src_mask):
        # Encode Inputs
        src_label = self.encode_input(src_label)
        src_image = self.encode_input(src_image)
        trg_template = self.encode_input([trg_template])
        src_mask = self.encode_input(src_mask)
